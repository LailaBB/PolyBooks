...
$(document).ready(function(){
    ...
    if(typeof module !== "undefined"){
        $('#module option[value='+module+']').prop('selected', true);
    }

    $( "#module" ).change(function() {
            if(document.URL.indexOf("?module") < 1)
                window.location.replace(document.URL + '?module=' + $( this ).val());
            else{
                var url = document.URL.replace("?module=" +module, "?module=" + $( this ).val());
...
function fillLibrary(library) {
   // assembleData();
    ...
    for (i=0; i < library.length; i++) {
        ...
        html += '<h2>Publié par ' + book.createdBy + '</h2>';
        html += '<p>' + book.description + '</p> <a style="width:3px;" target="_blank" href="'+ book.document + '"><img title="Télécharger" src="http://icons.iconarchive.com/icons/martz90/circle-addon2/24/downloads-icon.png"></a>';
        html += '</div></li>';
        $('.library').append(html);
